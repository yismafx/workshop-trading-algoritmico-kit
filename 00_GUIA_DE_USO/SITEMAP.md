# 🗺️ SITEMAP - Mapa de Navegación del Kit

**Última actualización:** Noviembre 2025 • **Versión:** 2.1

---

## 🏠 INICIO

**Punto de entrada principal:**
- [README.md](../README.md) - Descripción general del workshop

---

## 📂 ESTRUCTURA DEL REPOSITORIO

```
workshop-trading-algoritmico-kit/
│
├── 📄 README.md                    → Página principal
├── 📄 _config.yml                  → Configuración GitHub Pages
│
└── 📂 00_GUIA_DE_USO/             → Documentación completa
    ├── 📖 GUIA_INICIO.md          → ⭐ Guía completa de inicio
    ├── ⚙️ Guia_Setup_Completa.md  → Instrucciones de configuración
    ├── ⚡ Setup_A_Colab_Rapido.md   → Setup rápido para Google Colab
    ├── 📋 Programa_Detallado_Workshop.md → Estructura de 9 sesiones
    ├── 🗺️ Mapa_Sistema_Trading.html → Diagrama interactivo
    ├── 📓 Setup_y_Practica_Trading.ipynb → Notebook de práctica
    ├── 📄 SITEMAP.md              → Este archivo
    └── 📄 GLOSARIO_NAVEGACION.md  → Índice de nombres de archivos
```

---

## 🎯 RUTAS DE NAVEGACIÓN RECOMENDADAS

### 🟦 Ruta 1: Nuevo Participante (Primera Vez)

**Objetivo:** Entender el workshop y configurar el ambiente

```
1. README.md
   ↓
2. GUIA_INICIO.md (Sección "¿Por Dónde Empezar?")
   ↓
3. Programa_Detallado_Workshop.md (Ver estructura completa)
   ↓
4. Guia_Setup_Completa.md (Decidir qué setup usar)
   ↓
5. Setup_A_Colab_Rapido.md (Recomendado para empezar)
   ↓
6. GUIA_INICIO.md (Volver para siguiente paso)
```

**Tiempo estimado:** 60-90 minutos

---

### 🟨 Ruta 2: Setup Rápido (Quiero Empezar Ya)

**Objetivo:** Estar listo para la primera sesión en <50 minutos

```
1. Setup_A_Colab_Rapido.md
   ↓
2. Ejecutar notebook: Setup_y_Practica_Trading.ipynb
   ↓
3. Validar instalación
   ↓
4. ✅ Listo para S1
```

**Tiempo estimado:** 30-50 minutos

---

### 🟩 Ruta 3: Consulta Durante el Workshop

**Objetivo:** Resolver dudas específicas mientras avanzas

```
GUIA_INICIO.md
    ├─→ FAQ (Sección "Preguntas Frecuentes")
    ├─→ Troubleshooting (Errores comunes)
    └─→ Recursos Adicionales
```

**Tiempo estimado:** 5-15 minutos por consulta

---

## 📚 ÍNDICE ALFABÉTICO DE DOCUMENTOS

| Documento | Ubicación | Propósito | Estado |
|-----------|-----------|-----------|--------|
| **GUIA_INICIO.md** | `00_GUIA_DE_USO/` | ⭐ Guía maestra - Punto de partida | ✅ |
| **GLOSARIO_NAVEGACION.md** | `00_GUIA_DE_USO/` | Índice de nombres de archivos | ✅ |
| **Guia_Setup_Completa.md** | `00_GUIA_DE_USO/` | Instrucciones completas de setup | ✅ |
| **Mapa_Sistema_Trading.html** | `00_GUIA_DE_USO/` | Diagrama interactivo del sistema | ✅ |
| **Programa_Detallado_Workshop.md** | `00_GUIA_DE_USO/` | Estructura de 9 sesiones | ✅ |
| **README.md** | Raíz del repo | Descripción general del workshop | ✅ |
| **Setup_B_Python_Local.md** | `00_GUIA_DE_USO/` | Setup con Python local | 🟡 |
| **Setup_C_MetaTrader5.md** | `00_GUIA_DE_USO/` | Integración con MetaTrader 5 | 🟡 |
| **Setup_A_Colab_Rapido.md** | `00_GUIA_DE_USO/` | Setup rápido para Google Colab | ✅ |
| **Setup_y_Practica_Trading.ipynb** | `00_GUIA_DE_USO/` | Notebook de práctica | ✅ |
| **SITEMAP.md** | `00_GUIA_DE_USO/` | Este archivo - Mapa de navegación | ✅ |
| **Troubleshooting_Comun.md** | `00_GUIA_DE_USO/` | Guía de resolución de problemas | 🟡 |

---

## 🔗 LINKS EXTERNOS IMPORTANTES

### GitHub Pages
- [Mapa Mental Interactivo](https://yismafx.github.io/workshop-trading-algoritmico-kit/00_GUIA_DE_USO/Mapa_Sistema_Trading.html)

### Contacto
- 📧 Email: yismaryme@gmail.com
- 💬 Telegram: [@yismary](https://t.me/yismary)
- 📱 Canal: Fractals Market

### Repositorio
- 🔗 GitHub: [yismafx/workshop-trading-algoritmico-kit](https://github.com/yismafx/workshop-trading-algoritmico-kit)

---

## ⚠️ ARCHIVOS EN DESARROLLO (v2.2)

Los siguientes archivos están disponibles como **placeholders** en v2.1, pero su contenido completo estará disponible en v2.2:

### Setup Alternativos
- `Setup_B_Python_Local.md` - 🟡 Placeholder disponible
- `Setup_C_MetaTrader5.md` - 🟡 Placeholder disponible

### Documentación Adicional
- `Troubleshooting_Comun.md` - 🟡 Placeholder disponible

**Nota:** Los placeholders incluyen:
- ✅ Navegación funcional (breadcrumbs y links)
- ✅ Descripción de contenido futuro
- ✅ Alternativas disponibles ahora
- ✅ Roadmap y estimación de disponibilidad

---

## 🆘 ¿ESTÁS PERDIDO?

Si no sabes por dónde empezar o a dónde ir:

1. **Primera vez aquí:** [GUIA_INICIO.md](GUIA_INICIO.md) - Sección "¿Por Dónde Empezar?"
2. **Tengo una duda específica:** [GUIA_INICIO.md](GUIA_INICIO.md) - Sección FAQ
3. **Necesito configurar:** [Guia_Setup_Completa.md](Guia_Setup_Completa.md)
4. **Quiero ver el programa:** [Programa_Detallado_Workshop.md](Programa_Detallado_Workshop.md)
5. **Busco un archivo específico:** [GLOSARIO_NAVEGACION.md](GLOSARIO_NAVEGACION.md)

---

## 📊 ESTADO DEL KIT

**Versión actual:** 2.1  
**Última actualización:** Noviembre 2025  
**Documentos disponibles:** 16/16 (100%)  
**Completitud:** ✅ Navegación completa, 🟡 Contenido en desarrollo

### ✅ Completado (v2.1)
- [x] Documentación central (GUIA_INICIO, Guia_Setup_Completa, Programa_Detallado)
- [x] Setup básico (Colab Rápido)
- [x] Mapa mental interactivo
- [x] Navegación estandarizada (SITEMAP, GLOSARIO, breadcrumbs)
- [x] Placeholders profesionales (7 documentos)
- [x] **0 links rotos** ✅

### 🟡 En Desarrollo (v2.2)
- [ ] Contenido completo de setups alternativos (B, C, D)
- [ ] Guía completa del Colab Notebook Maestro
- [ ] Troubleshooting detallado
- [ ] Documentación exhaustiva de librerías

### 🟢 Futuro (v3.0)
- [ ] Templates profesionales
- [ ] Prompts Library completa
- [ ] Scripts auxiliares
- [ ] Casos de estudio

---

## 🔄 CHANGELOG

### v2.1 (Noviembre 2025)
**Fase 2:** Eliminación completa de links rotos
- Creación de 7 placeholders profesionales
- Actualización de SITEMAP y GLOSARIO
- 0 links rotos (100% navegación funcional)

**Fase 1:** Correcciones críticas de navegación
- Estandarización de nombres de archivo
- Creación de SITEMAP.md y GLOSARIO_NAVEGACION.md
- Corrección de 29 links rotos (TIPO 1)
- Actualización de README.md

### v2.0 (Octubre 2025)
- Estructura inicial del repositorio
- Documentación central
- Mapa mental interactivo

---

**Fin del Sitemap**

---

🏠 [Volver a GUIA_INICIO.md](GUIA_INICIO.md) | 📖 [Ver Glosario](GLOSARIO_NAVEGACION.md) | 🔝 [Inicio del Sitemap](#-sitemap---mapa-de-navegación-del-kit)
