# Workshop: Trading Algorítmico Aumentado con IA Generativa

**Transforma tu trading manual en sistemas algorítmicos potenciados por IA**

---

## Sobre el Workshop

Este workshop está diseñado para **traders manuales con experiencia** que quieren:

- ✅ Sistematizar su edge con algoritmos
- ✅ Escalar sin limitaciones de tiempo  
- ✅ Aumentar capacidades con IA Generativa

---

## Documentación

### Inicio Rápido

1. **[📖 LEEME_PRIMERO.md](00_GUIA_DE_USO/LEEME_PRIMERO.md)** - Comienza aquí
2. **[🗺️ Mapa Mental Interactivo](https://yismafx.github.io/workshop-trading-algoritmico-kit/00_GUIA_DE_USO/MAPA_MENTAL_SISTEMA_TRADING_PNG.html)** - Visualiza el sistema
3. **[⚙️ Instrucciones de Setup](00_GUIA_DE_USO/Instrucciones_Setup_Ambiente.md)** - Configura tu ambiente

### Contenido del Kit Público
```
📦 workshop-trading-algoritmico-kit/
│
├── 📂 00_GUIA_DE_USO/          → Documentación
├── 📂 02_TEMPLATE_PACK/        → Templates profesionales
├── 📂 03_PROMPTS_LIBRARY/      → Prompts para GenAI
└── 📂 04_SCRIPTS_AUXILIARES/   → Código reutilizable
```

---

## Arquitectura del Sistema

El sistema tiene **5 componentes principales** aumentados por IA:

- **🗄️ DATA PIPELINE** - Descarga y limpia datos históricos
- **🧠 ESTRATEGIA** - Genera señales de trading
- **🛡️ RISK MANAGEMENT** - Protección de capital
- **🔬 BACKTESTING** - Validación histórica
- **⚙️ EXECUTION** - Trading en vivo

**[Ver Diagrama Completo →](https://yismafx.github.io/workshop-trading-algoritmico-kit/00_GUIA_DE_USO/MAPA_MENTAL_SISTEMA_TRADING_PNG.html)**

---

## Estructura del Workshop

### Semana 1: Fundamentos

- **S1:** Fundamentos y Expectativas Realistas ✅
- **S2:** Data Pipeline y Quality Forensics ✅
- **S3:** Ideación con GenAI 🔄

### Semana 2: Implementación

- **S4:** Implementación de Estrategias
- **S5:** Backtesting Riguroso
- **S6:** Risk Management

### Semana 3: Producción

- **S7:** Multi-Plataforma
- **S8:** Deployment
- **S9:** Mantenimiento

**Total:** 9 sesiones • 27 horas • 3 semanas

---

## Contenido Premium vs Público

### ✅ Contenido Público (Este Repo)

- Documentación completa
- Templates profesionales
- Scripts auxiliares (open source)
- Prompts básicos
- Mapa mental interactivo

### 🔒 Contenido Premium (Workshop Pagado)

- **9 Colab Notebooks completos**
- **Videos de todas las sesiones**
- **Estrategias backtestadas**
- **35+ prompts avanzados**
- **Soporte directo (Telegram)**
- **Casos de estudio reales**

---

## Expectativas Realistas

> Este workshop NO te hará millonario. Te dará un PROCESO robusto y repetible.

**Objetivos realistas:**

- **Sharpe Ratio:** 1.0 - 1.5 (no 3.0+)
- **Max Drawdown:** 20% - 30% (no menos de 5%)
- **Tasa de éxito:** 2 de cada 10 estrategias funcionan
- **Tiempo:** 3-6 meses hasta primera estrategia rentable (no 1 semana)

---

## Stack Tecnológico

### Lenguajes

- Python 3.10+
- Pine Script (TradingView)
- MQL5 (MetaTrader)

### Librerías Principales

- `pandas`, `numpy` - Manipulación de datos
- `yfinance`, `alpaca-trade-api` - Descarga de datos
- `backtrader`, `vectorbt` - Backtesting
- `matplotlib`, `seaborn` - Visualización

### Plataformas

- Google Colab - Notebooks sin instalación
- TradingView - Desarrollo visual
- Alpaca - Paper y live trading
- Interactive Broker

### IA Generativa

- Claude (Anthropic) - Recomendado
- ChatGPT (OpenAI)
- GitHub Copilot

---

## Inscripciones

### Inversión

- **Precio:** Solicitar informacion
- **Incluye:** 27 horas + materiales premium
- **Pago:** Solicitar informacion

### Próxima Cohorte

- **Formato:** Virtual (Google Meet)
- **Cupo:** Limitado
- **Inicio:** A confirmar

### Contacto

- 📧 **Email:** [yismaryme@gmail.com](mailto:yismaryme@gmail.com)
- 💬 **Telegram:** [@yismary](https://t.me/yismary)

---

## Contribuciones

Este repositorio es **parcialmente open-source**:

**Puedes:**
- Reportar bugs (Issues)
- Sugerir mejoras (Issues)
- Contribuir código (Pull Requests)

**No puedes:**
- Vender o redistribuir material premium
- Crear workshops derivados sin permiso

---

## Recursos Adicionales

### Libros Recomendados

1. Chan, E. (2013) - *Algorithmic Trading*
2. Carver, R. (2015) - *Systematic Trading*
3. López de Prado, M. (2018) - *Advances in Financial ML*

### Comunidades

- Fractals Market

---

## Disclaimer

⚠️ **Material para fines educativos únicamente**

- NO constituye asesoría de inversión
- Trading implica riesgo de pérdida de capital
- Resultados pasados NO garantizan resultados futuros
- Nunca operes con dinero que no puedas perder

---

## ⭐ Apoya Este Proyecto

Si este repositorio te resulta útil:
- Dale una estrella al repo
- Compártelo con otros traders
- Únete a las discusiones

---

**Última actualización:** Noviembre 2025 • **Versión:** 1.0 (S1-S2 completas)

**Mantenido por:** [@yismafx](https://github.com/yismafx)