# Workshop: Trading Algorítmico Aumentado con IA Generativa

> ⚠️ **DISCLAIMER IMPORTANTE**  
> Este material es parte del Workshop "Trading Algorítmico Aumentado con IA Generativa".  
> El contenido es exclusivamente educativo. NO constituye asesoría financiera.  
> El trading implica riesgo de pérdida de capital. Opera solo con capital que puedas perder.

**Transforma tu trading manual en sistemas algorítmicos potenciados por IA**

> 🎯 **Principio Fundamental:** GenAI = Copiloto, NO Piloto Automático

---

## 🚀 ACCESO RÁPIDO

### 👤 **Ya soy participante del workshop**

**Accede directamente al kit:**

- 📖 **[Guía de Inicio Completa →](00_GUIA_DE_USO/GUIA_INICIO.md)** - Tu onboarding completo (17 min)
- 🗺️ **[Mapa Mental Interactivo →](https://yismafx.github.io/workshop-trading-algoritmico-kit/00_GUIA_DE_USO/Mapa_Sistema_Trading.html)** - Visualiza la arquitectura
- ⚙️ **[Setup Rápido →](00_GUIA_DE_USO/Setup_A_Colab_Rapido.md)** - Google Colab + Alpaca (15 min)
- 🚨 **[Troubleshooting →](00_GUIA_DE_USO/Troubleshooting_Maestro.md)** - Resuelve problemas
- 🤔 **[FAQ Completo →](00_GUIA_DE_USO/FAQ_COMPLETO.md)** - 35 preguntas

---

### 🤔 **Estoy evaluando el workshop**

**Información clave para decidir:**

- 🎓 **[¿Es para mí?](#-para-quién-es-este-workshop)** - Perfil y pre-requisitos
- 📋 **[Programa](#-estructura-del-workshop)** - 9 sesiones, 27 horas
- ⚖️ **[Expectativas](#️-expectativas-realistas-anti-hype)** - Anti-hype transparente
- 💰 **[Inversión](#-inversión)** - Precio y opciones
- 📞 **[Contacto](#-contacto-e-inscripciones)** - Inscríbete

---

## 📚 Navegación Completa

**Documentación:**
- 📖 [Guía de Inicio](00_GUIA_DE_USO/GUIA_INICIO.md) - Onboarding completo
- 🗺️ [Mapa del Sitio](00_GUIA_DE_USO/SITEMAP.md) - Estructura
- 📚 [Programa Detallado](00_GUIA_DE_USO/Programa_Detallado_Workshop.md) - 9 sesiones

**Recursos:**
- 📂 [Setup Completo](00_GUIA_DE_USO/Guia_Setup_Completa.md) - Config avanzada
- 🚨 [Troubleshooting](00_GUIA_DE_USO/Troubleshooting_Maestro.md) - Soluciones
- 📝 [Templates](02_TEMPLATE_PACK/) - Documentos profesionales
- 🤖 [Prompts](03_PROMPTS_LIBRARY/) - Biblioteca

---

## 🎓 ¿Para Quién es Este Workshop?

### ✅ Perfil Ideal

**Traders Manuales con Experiencia**
- Ya operas con estrategias propias (no principiantes)
- Conoces gestión de riesgo y psicología del trading
- Quieres sistematizar tu edge con IA

**Analistas Financieros**
- Buscas construir modelos sistemáticos
- Quieres ir más allá de reportes estáticos

### ❌ NO es para ti si:

- Buscas "hacerte rico rápido"
- No tienes experiencia operando en mercados reales
- Esperas que la IA haga todo el trabajo
- No estás dispuesto a invertir 3-6 meses en desarrollo riguroso

### ✅ Lo Que NO Necesitas

- **NO necesitas ser programador** - Te damos código ya funcionando
- **NO necesitas experiencia en IA** - Te damos 35+ prompts probados
- **NO necesitas instalaciones complejas** - Todo en Colab Notebooks

---

## 🗺️ Estructura del Workshop

**Duración:** 27 horas • **Formato:** 9 sesiones de 3h • **Ritmo:** 3 sesiones/semana

| Semana | Sesiones | Enfoque | Entregable |
|--------|----------|---------|------------|
| 🟦 **Semana 1** | S1-S3 | **Fundamentos + Datos + Ideación** | Dataset limpio + 3 hipótesis |
| 🟨 **Semana 2** | S4-S6 | **Implementación + Backtesting + Riesgo** | Estrategia validada con WFO |
| 🟩 **Semana 3** | S7-S9 | **Multi-Plataforma + Deployment** | Bot en paper trading activo |

**[📋 Ver programa detallado completo →](00_GUIA_DE_USO/Programa_Detallado_Workshop.md)**

---

## ⚖️ Expectativas Realistas (Anti-Hype)

> ⚠️ Este workshop NO te hará millonario. Te dará un PROCESO robusto y repetible.

| Métrica | Objetivo Realista | ❌ Expectativa Irreal |
|---------|-------------------|----------------------|
| **Sharpe Ratio** | 1.0 - 1.5 | 3.0+ |
| **Max Drawdown** | 20% - 30% | < 5% |
| **Tasa de éxito** | 2 de 10 estrategias | Todas funcionan |
| **Tiempo desarrollo** | 3-6 meses | 1 semana |

**Estadísticas honestas:**
- 90% de estrategias fallan en validación rigurosa (nuestro objetivo: ser el 10%)
- Drawdowns del 20-30% son normales en estrategias robustas
- El desarrollo toma meses, no días

---

## 📦 Lo Que Incluye el Kit

### ✅ Contenido Público (Este Repo)

- 📚 Documentación completa
- 📄 Templates profesionales (Strategy Memo, Backtest Report)
- 🐍 Scripts auxiliares (Python)
- 🤖 Prompts básicos para GenAI

### 🔒 Contenido Premium (Participantes Pagos)

- 📓 **Colab Notebook Maestro** (código completo ejecutable)
- 🎥 **Videos grabados** de 27 horas
- 📊 **Estrategias backtestadas** (código + resultados WFO)
- 🤖 **Biblioteca 35+ prompts avanzados**
- 💬 **Soporte directo** (Telegram privado)
- 📚 **Playbook de Crisis Management**

---

## 🛠️ Stack Tecnológico

**Lenguajes (Usarás, No Programarás):**  
Python 3.10+, Pine Script, MQL5

**Librerías (Preconfiguradas):**  
yfinance, alpaca-trade-api, backtrader, vectorbt, matplotlib, pandas

**Plataformas:**  
Google Colab (sin instalación), TradingView, Alpaca, Interactive Brokers

**IA Generativa (Tu Copiloto):**  
Claude (recomendado), ChatGPT, GitHub Copilot

---

## 💰 Inversión

**Precio:** $950 USD (Descuento para miembros Fractals Market)  
**Incluye:** 27h formación + Colab Notebook + Templates + Prompts + Soporte

**Opciones de pago:** Consultar disponibles  
**Próxima cohorte:** Consultar fechas  
**Formato:** Virtual (Google Meet)  
**Cupo:** Consultar disponibilidad

---

## 🧭 Filosofía: Python Primero

### 🤔 "¿Por qué Python si yo opero en TradingView/MetaTrader?"

**Respuesta honesta:**

Python es el **"Lenguaje Universal"** del Trading Algorítmico.

En Trading Manual, aprendes análisis técnico (RSI, MACD, Bandas Bollinger) **una vez** y lo aplicas en **cualquier plataforma**.

**En Trading Algorítmico es igual:**

```
Semanas 1-2: Fundamentos en Python
    ├─ Descargas datos
    ├─ Diseñas estrategias con IA
    ├─ Backtesting riguroso
    └─ Gestión de riesgo profesional

→ Semana 3: Multi-Plataforma ⭐ AQUÍ LA MAGIA
    ├─ Python → Pine Script (TradingView)
    ├─ Python → MQL5 (MetaTrader)
    └─ IA hace la traducción automática
```

**Ventajas del enfoque Python Primero:**

✅ Desarrollo riguroso y validación profesional  
✅ Walk-Forward Analysis, Monte Carlo  
✅ Portabilidad total del código  
✅ Ejecución en la plataforma que prefieras

> **Confía en el proceso.** Miles de traders profesionales han seguido este camino.

---

## 📚 Referencias Bibliográficas

1. **Chan, E. (2013)** - *Algorithmic Trading: Winning Strategies and Their Rationale*
2. **Carver, R. (2015)** - *Systematic Trading: A Unique New Method for Designing Trading and Investing Systems*
3. **López de Prado, M. (2018)** - *Advances in Financial Machine Learning*
4. **Strimpe, J. (2024)** - *Python for Algorithmic Trading Cookbook*

---

## 📞 Contacto e Inscripciones

**📧 Email:** yismaryme@gmail.com  
**💬 Telegram:** [@yismary](https://t.me/yismary)  
**📱 Canal:** Fractals Market

**Antes de contactar:**
- Lee la [Guía de Inicio](00_GUIA_DE_USO/GUIA_INICIO.md) completa
- Revisa el [FAQ](00_GUIA_DE_USO/FAQ_COMPLETO.md)
- Consulta [Troubleshooting](00_GUIA_DE_USO/Troubleshooting_Maestro.md) para problemas técnicos

---

## 🤝 Contribuciones

**Puedes:**
- Reportar bugs (Issues)
- Sugerir mejoras (Issues)
- Contribuir código público (Pull Requests)

**No puedes:**
- Vender o redistribuir material premium
- Crear workshops derivados sin permiso

---

## ⚠️ Disclaimer Legal

**Material para fines educativos únicamente.**

❌ NO constituye asesoría de inversión  
⚠️ Trading algorítmico implica riesgo de pérdida de capital  
📊 Resultados pasados NO garantizan resultados futuros  
💰 Nunca operes con dinero que no puedas perder

**Como todo trading:**
- El rendimiento pasado no garantiza resultados futuros
- Toda estrategia puede fallar
- La gestión de riesgo es crítica

---

## ⭐ Apoya Este Proyecto

- Dale una estrella al repo
- Compártelo con otros traders
- Únete a las discusiones (Issues)

---

**Versión:** 3.0 | **Última actualización:** 17 de noviembre de 2025  
**Estado:** ✅ Validado y listo para uso  
**Mantenido por:** [@yismary](https://github.com/yismary) | **Comunidad:** Fractals Market

---

**¿Listo para comenzar?** 🚀  
👉 **[Accede al Kit →](00_GUIA_DE_USO/GUIA_INICIO.md)**
